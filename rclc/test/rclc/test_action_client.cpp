...
protected:
  ...
  std::function<void(const std::shared_ptr<GoalHandleFibonacci> goal_handle)> server_handle_accepted;
};
...
{
  uint64_t lhs_high = *((uint64_t *) &lhs.uuid[0]);
  uint64_t lhs_low = *((uint64_t *) &lhs.uuid[8]);

  uint64_t rhs_high = *((uint64_t *) &rhs.uuid[0]);
  uint64_t rhs_low = *((uint64_t *) &rhs.uuid[8]);
